...
func NewConveyorRepository(db *gorm.DB) ConveyorRepository {
	return &conveyorRepository{db: db}
}

func (r *conveyorRepository) WithTransaction(ctx context.Context, fn func(ConveyorRepository) error) error {
...
func (r *conveyorRepository) ListProjectReportSources(ctx context.Context, projectID uuid.UUID, start time.Time, end time.Time) ([]ProjectReportSource, error) {
	...
	out := make([]ProjectReportSource, 0, len(tasks))
	for _, task := range tasks {
		...
		out = append(out, ProjectReportSource{Task: task, Events: events, Evidence: evidence})
	}
	return out, nil
}
